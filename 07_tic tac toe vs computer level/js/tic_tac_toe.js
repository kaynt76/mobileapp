...
function isReach(status){
    ...
    lineArray.some(function(line){
        let poroCheckCnt = 0;
        let chonccCheckCnt = 0;

        line.forEach(function(square){
            if (square.classList.contains("js-poro-checked")){
                poroCheckCnt++;
            }
            if (square.classList.contains("js-choncc-checked")){
                chonccCheckCnt++;
            }
        });

        if (status === "poro" && poroCheckCnt === 2 && chonccCheckCnt ===0){
            poroTurnEnd = "1";
        }
        if (status === "choncc" && poroCheckCnt === 0 && chonccCheckCnt ===2){
            poroTurnEnd = "1";
        }
